...
def _mk_label(parent, text) -> ctk.CTkLabel:
    return ctk.CTkLabel(
        parent, text=text, font=("Courier New", 14, "bold"), text_color=TEXT_LABEL
    )


def _mk_btn(
    parent, text, cmd, fg=BTN_BG, hv=BTN_HOVER, tc=BTN_TEXT, w=160
) -> ctk.CTkButton:
...
class _ArrayCanvas(tk.Canvas):
    ...
    def _render(self) -> None:
        ...
        # Rail exterior
        self._rrect(
            rx0, ry0, rx1, ry1, 14, fill=CELL_OUTER, outline=CELL_BORDER, width=2
        )
        ...
        for sx in [rx0 + 7, rx1 - 17]:
            ...
            self.create_rectangle(
                sx, my - 20, sx + 10, my + 20, fill=CELL_NOTCH, outline=""
            )
    ...
    def _draw_cell(self, x0, y0, x1, y1, idx):
        ...
        self.create_rectangle(
            nx - nw + 7, y0 + 3, nx + nw - 7, y0 + nh - 3, fill=CELL_NOTCH, outline=""
        )
...
class MatteApp:
    ...
    def _build_layout(self) -> None:
        ...
        self._header = ctk.CTkFrame(
            self.root, fg_color=BG_TOP, corner_radius=0, height=130
        )
        ...
        self.info_lbl = ctk.CTkLabel(
            self._header, text="", font=("Courier New", 13), text_color=TEXT_SUB
        )
        ...
        self._footer = ctk.CTkFrame(
            self.root, fg_color=BG_PANEL, corner_radius=0, height=110
        )
        self._footer.pack(fill="x", side="bottom")
        self._footer.pack_propagate(False)
        tk.Canvas(self._footer, height=2, bg=ACCENT, highlightthickness=0).pack(
            fill="x"
        )
    ...
    def _show_create_controls(self) -> None:
        ...
        ctk.CTkOptionMenu(
            ...
            fg_color=INPUT_BG,
            button_color=BORDER,
            button_hover_color=HATCH,
            text_color=INPUT_FG,
            dropdown_fg_color=INPUT_BG,
            dropdown_text_color=INPUT_FG,
            dropdown_hover_color=HATCH,
            font=("Courier New", 14),
    ...
    def _show_category_menu(self) -> None:
        self._title_lbl.configure(
            text="[ ARRAY ENGINE OPERATIONS DASHBOARD ]",
    ...
    def _show_insert_options(self) -> None:
        ...
        _mk_btn(
            ...
            tc=INPUT_FG,
            w=110,
        ).grid(row=0, column=len(ops), padx=6)
    ...
    def _show_insert_panel(self, mode: str) -> None:
        ...
        _mk_btn(
            self.ctrl,
            "← Back",
            self._show_insert_options,
            fg=BORDER,
            hv=ACCENT,
            tc=INPUT_FG,
            w=110,
        ).grid(row=0, column=col + 1, padx=0)
    ...
    def _show_remove_options(self) -> None:
        ...
        for j, (name, fn) in enumerate(ops):
            _mk_btn(
                self.ctrl, name, fn, fg=BTN_RED, hv=BTN_RED_HV, tc=BTN_RED_TXT, w=140
            ).grid(row=0, column=j, padx=6)
        _mk_btn(
            self.ctrl,
            "← Back",
            self._show_category_menu,
    ...
    def _show_sort_options(self) -> None:
        ...
        _mk_btn(
            ...
            tc=INPUT_FG,
            w=110,
        ).grid(row=0, column=len(algos), padx=6)
    ...
    def _show_get_options(self) -> None:
        ...
        _mk_btn(self.ctrl, "▶ Get", lambda: self._do_get(idx_var.get()), w=140).grid(
